...
- **⚠️ Email rate-limit (BLOKKER bij bulk)**: Supabase's ingebouwde mailer doet 3 mails/uur. Bij bulk-onboarding (CSV met 100+ rijen + per-rij "Stuur uitnodiging") loopt dat na een paar klikken vast met `over_email_send_rate_limit`. **Configureer eerst een externe SMTP-provider** (Resend/SendGrid/SES) — zie `docs/PORTAAL-OVERZICHT.md` § 6.
